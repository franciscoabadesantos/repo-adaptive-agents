...
class TestCatalogTypesAuditAndThinRunner:
    ...
    def test_thin_runner_calls_native_and_hides_sensitive_invalid_content(self):
        path = ROOT / "experiments/admission_control/native_boundary/thin_runner.py"
        spec = importlib.util.spec_from_file_location("native_thin_runner", path)
        assert spec and spec.loader
        runner = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(runner)
        hidden = _resource(
            "hidden", state=native.LifecycleState.DRAFT,
            exposure_policy=native.ExposurePolicy.REQUIRE_ADMISSIBLE,
            body="must never reach callback",
        )
        visible = _resource("visible")
        catalog = _catalog(hidden, visible)
        observed = []

        def retrieve(resources):
            observed.append(tuple(resource.id for resource in resources))
            return resources

        def select(context, resources):
            assert context["effective_at"] == "2026-09-01T12:00:00Z"
            assert all("must never reach callback" not in r.body for r in resources)
            return _select("visible", "hidden")

        trace = runner.run_case(_context().model_facing_data(), catalog.canonical_data(), retrieve, select)
        assert observed == [("visible",)]
        assert trace["decision_origin"] == {
        ...
        assert [item["resource_id"] for item in validation["final_resources"]] == ["visible"]
        assert {r["code"] for r in validation["selection_decisions"][1]["reasons"]} >= {
            "not_exposed", "not_approved",
        }

    def test_thin_runner_source_has_no_policy_helpers_or_benchmark_vocabulary(self):
        source = (ROOT / "experiments/admission_control/native_boundary/thin_runner.py").read_text()
        forbidden = (
            "evaluate_lifecycle", "evaluate_scope", "evaluate_compatibility",
            "resolve_precedence", "resolve_supersession",
        )
        assert all(term not in source for term in forbidden)
        assert "native.admit(" in source
        assert "native.validate(" in source
